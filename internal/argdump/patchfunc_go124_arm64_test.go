	...
	*argdump.DebugEnabled =os.Getenv("ARGDUMP_DEBUG") == "1"
	...
	if *argdump.DebugEnabled {
		...
		for i := 0; i+4 <= len(code); i += 4 {
			// arm64asm.Decode accepts up to 16 bytes; pass what's available.
			chunk := code[i:]
			if len(chunk) > 16 {
				chunk = chunk[:16]
			}
			ins, derr := arm64asm.Decode(chunk)
			if derr != nil || ins.Op == 0 {
				break
			}
			_, _ = os.Stderr.WriteString(
				"  0x" + strconv.FormatUint(uint64(origin)+uint64(i), 16) + ":\t" + ins.Op.String() + "\t" + ins.String() + "\n",
			)
		}
		_, _ = os.Stderr.WriteString(
